...
openssl genrsa -out "$workdir/ca.key" 3072 >/dev/null 2>&1 || error_exit 1
...
openssl genrsa -out "$workdir/server.key" 3072 >/dev/null 2>&1 || error_exit 1
...
openssl genrsa -out "$workdir/client.key" 3072 >/dev/null 2>&1 || error_exit 1
