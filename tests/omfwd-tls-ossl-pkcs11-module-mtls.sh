...
openssl genrsa -out "$workdir/intermediate.key" 2048 >/dev/null 2>&1 || error_exit 1
openssl req -new -key "$workdir/intermediate.key" \
	-subj "/C=US/ST=CA/L=Test/O=rsyslog/OU=test/CN=Intermediate-CA" \
	-out "$workdir/intermediate.csr" >/dev/null 2>&1 || error_exit 1
openssl x509 -req -in "$workdir/intermediate.csr" -CA "$workdir/ca.pem" -CAkey "$workdir/ca.key" \
	...
	-out "$workdir/intermediate.pem" >/dev/null 2>&1 || error_exit 1
...
openssl x509 -in "$workdir/ca.pem" -outform DER -out "$workdir/ca.der" >/dev/null 2>&1 || error_exit 1
openssl x509 -in "$workdir/intermediate.pem" -outform DER -out "$workdir/intermediate.der" >/dev/null 2>&1 || error_exit 1
...
pkcs11-tool --module "$softhsm_module" --login --pin "$token_pin" --token-label "$token_label" \
	--write-object "$workdir/ca.der" --type cert --id 02 --label ca-cert >/dev/null 2>&1 || error_exit 1
pkcs11-tool --module "$softhsm_module" --login --pin "$token_pin" --token-label "$token_label" \
	--write-object "$workdir/intermediate.der" --type cert --id 03 --label intermediate-ca-cert >/dev/null 2>&1 || error_exit 1
